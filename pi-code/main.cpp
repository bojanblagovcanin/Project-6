...
{
	std::string command = "mpg123 " + audioFile;
			...
			case 3:
				...
				while(1){			
					...
					if (prev_floorNumber != floorNumber) {								// If floor number changes in database
						...
						std::string audioFile = "mpg123/floor_" + std::to_string(floorNumber) + ".mp3";
						playAudio(audioFile);					// change floor number in elevator - send command over CAN
					}
